    ...
    def move2(self, steps):
        def action(next_done):
            def step():
                ...
                # ✅ move2 不畫線，直接跳過 trace
                # self._draw_trace(from_x, from_y, self.x, self.y)  ← 把這行拿掉
...
def start(ev):
    global w, r, length, width, steps, steps1
    length = int(document["input_length"].value)
    width = int(document["input_width"].value)
    steps = length - 1
    steps1 = 1

    w = World(length, width)
    r = AnimatedRobot(w, 1, 1)
    r.enable_trace()  # 預設不畫線

def run_code(ev):
    global w, r, length, width, steps, steps1
    code = document["code_area"].value
    try:
        exec(code, globals())
    except Exception as e:
        print("程式執行錯誤:", e)

document["btn_start"].bind("click", start)
document["run_code_btn"].bind("click", run_code)
// ]]></script>